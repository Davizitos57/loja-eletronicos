...
export default function AdminList() {
  ...
  const loadData = React.useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const listData = await getAdmins({
        paginationModel,
        sortModel,
        filterModel,
      });
  ...
  const handleRowClick = React.useCallback(
    ({ row }) => {
      navigate(`/administrador/${row.id}`);
    },
    [navigate],
  );

  const handleCreateClick = React.useCallback(() => {
    navigate('/administrador/new');
  }, [navigate]);

  const handleRowEdit = React.useCallback(
    (admin) => () => {
      navigate(`/administrador/${admin.id}/edit`);
    },
    [navigate],
  );
  ...
  const pageTitle = 'Administradores';
